...
from engine_finetune import train_one_epoch, evaluate, evaluate_or
...
def main(args):
    ...
    if args.eval:
        test_stats = evaluate_or(data_loader_val, model, device)#, class_idx_map, args.visualize_epoch, args.max_num)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if epoch % args.eval_epoch == 0:
            test_stats = evaluate_or(data_loader_val, model, device)
            print(f"Accuracy of the network on the {len(dataset_val)} test images: {test_stats['acc1']:.1f}%")
